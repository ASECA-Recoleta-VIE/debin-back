...
class DebinService(
    ...
    @Value("\${main-api.endpoints.transfer}") private val transferEndpoint: String
) {
    ...
    fun requestMoney(request: RequestMoneyRequest): ApiResponse<Any> {
        logger.info("Processing money request for account: ${request.accountIdentifier}")
        
        return try {
            val url = "$mainApiBaseUrl$transferEndpoint"
            
            val requestBody = mapOf(
                "accountIdentifier" to request.accountIdentifier,
                "amount" to request.amount,
                "description" to request.description,
                "requesterName" to request.requesterName,
                "requesterAccount" to request.requesterAccount,
                "source" to "DEBIN"
            )
            ...
            logger.info("Calling main API at: $url")
            ...
            logger.info("Main API response status: ${response.statusCode}")
            ...
            logger.error("Error calling main API", e)
